...
public final class AccountCommandHandler implements CommandHandler<String, AccountCommand, AccountEvent, Optional<Account>> {
    ...
    public static Result<CommandError, NonEmptyList<AccountEvent>> createAccountHandler(
        Optional<Account> currentAggregate, AccountCommand.CreateAccount command) {
        return currentAggregate
            .<Result<CommandError, NonEmptyList<AccountEvent>>>map(account -> Result.failure(CommandError.of(
                CommandError.Reason.CommandHandlerFailed,
                CreateAccountError.ACCOUNT_ALREADY_EXISTS.message())))
            .orElseGet(() -> {
                if (command.name == null || command.name.trim().isEmpty()) {
                    return Result.failure(CommandError.of(
                        CommandError.Reason.CommandHandlerFailed,
                        CreateAccountError.ACCOUNT_NAME_NOT_SET.message()));
                } else if (command.openingBalance < 0) {
                    return Result.failure(CommandError.of(
                        CommandError.Reason.CommandHandlerFailed,
                        CreateAccountError.OPENING_BALANCE_LESS_THAN_ZERO.message()));
                } else {
                    return Result.success(NonEmptyList.of(
                        new AccountEvent.AccountCreated(command.name, command.openingBalance, Instant.now())));
                }
            });
    }

    public static Result<CommandError, NonEmptyList<AccountEvent>> depositHandler(
        Optional<Account> currentAggregate, AccountCommand.Deposit command) {
        return currentAggregate
            ...
            .orElse(Result.failure(CommandError.of(
                CommandError.Reason.CommandHandlerFailed,
                "Account does not exist")));
    }

    public static Result<CommandError, NonEmptyList<AccountEvent>> withdrawHandler(
        Optional<Account> currentAggregate, AccountCommand.Withdraw command) {
        return currentAggregate
            .<Result<CommandError, NonEmptyList<AccountEvent>>>map(account -> {
                if (command.amount <= 0) {
                    return Result.failure(CommandError.of(
